...
var ProxyServer = function(config, matchers) {
  if (!config) {
    config = {};
  }
  ...
  this.rateLimitCode = config.rateLimitCode || 400;
  this.rateLimitMessage = config.rateLimitMessage || 'Rate limited.';
...
ProxyServer.prototype.identifyMatcher = function(detail) {
  ...
  for (var i = 0; i < this.matchers.length; i++) {
    ...
    if (result) {
      winston.verbose('using matcher: ' + matcher.name);
      return {
        tracker: result,
  ...
  detail.on('ready', function() {
    ...
    cache_manager.get(cacheKey, function(data) {
      ...
      // Otherwise, determine if we're allowed to hit the source
        ...
        winston.verbose('seeking match for url: ' + request.url);
        if (!m) {
          winston.warn('no match for url: ' + request.url);
          ...
          tracker.usesInInterval(function(num, total) {
            if (num >= total) {
              response.writeHead(_this.rateLimitCode, { 'content-type': 'application/json' });
              response.end(JSON.stringify({
                message: _this.rateLimitMessage,
                detail: {
                  current: num,
                  allowed: total,
                  interval: m.rateLimit.interval
                }
              }));
              winston.warn('request hit rate limit', { tracker: m.tracker });
            }
            else {
              _this.serve(proxy, response);
              tracker.used();
